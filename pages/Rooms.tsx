...
import { BedDouble, Wrench, Filter, Plus, Trash, User, AlertTriangle, UserMinus } from 'lucide-react';
...
const Rooms: React.FC = () => {
  const { data: rooms, loading: loadingRooms, setData: setRooms } = useSupabase<Room>('rooms');
  ...
  const handleDeleteRoom = (id?: string | number) => {
    ...
    // Check for tenants in this room
    const roomToDelete = rooms.find(r => r.id === id);
    if (roomToDelete) {
        const activeTenants = tenants.filter(t => 
             String(t.roomNumber).trim() === String(roomToDelete.number).trim() && 
             t.status !== 'Left'
        );
        
        if (activeTenants.length > 0) {
            alert(`Cannot Delete Room ${roomToDelete.number}\n\nIt is currently occupied by:\n${activeTenants.map(t => `- ${t.name}`).join('\n')}\n\nPlease go to the 'Tenants' page and Edit these tenants to "Unassign" them or Delete them first.`);
            return;
        }
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
        ...
        if (error.code === '23503') { 
            alert(`Cannot delete room because it is linked to records in database.`);
        } else if (error.code === '42501') {
            alert("Permission Denied: You cannot delete rooms. Check RLS policies.");
        } else {
  ...
  const handleToggleStatus = async (id: string | number, currentStatus: RoomStatus) => {
      if (isDemo) {
      ...
      if (currentStatus === RoomStatus.OCCUPIED) {
          ...
          if (room) {
             const occupants = tenants.filter(t => String(t.roomNumber).trim() === String(room.number).trim() && t.status !== 'Left');
             if (occupants.length > 0) {
                 alert(`This room is occupied by ${occupants.map(t => t.name).join(', ')}.`);
                 return;
             }
          }
      }
      
      const newStatus = currentStatus === RoomStatus.AVAILABLE ? RoomStatus.MAINTENANCE : RoomStatus.AVAILABLE;
  ...
  const handleFreeRoom = async (id: string | number) => {
      if (isDemo) return alert("Restricted in Demo Mode");

      if(confirm('Force mark this room as Available?')) {
          setRooms(prev => prev.map(r => r.id === id ? { ...r, status: RoomStatus.AVAILABLE } : r));
          await supabase.from('rooms').update({ status: RoomStatus.AVAILABLE }).eq('id', id);
      }
  };
  ...
  const getOccupants = (roomNumber: string) => {
      if (!tenants) return [];
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        {filteredRooms.map((room) => {
          ...
          const isGhostOccupancy = room.status === RoomStatus.OCCUPIED && occupants.length === 0;
          ...
          return (
            <div key={room.id} className="bg-card rounded-xl border border-border shadow-sm hover:border-primary/50 transition-all group overflow-hidden relative flex flex-col justify-between">
                <div className="absolute top-0 right-0 p-4 flex gap-2">
                    {room.status === RoomStatus.AVAILABLE && <div className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]"></div>}
                    {room.status === RoomStatus.OCCUPIED && <div className="w-2 h-2 rounded-full bg-destructive"></div>}
                    {room.status === RoomStatus.MAINTENANCE && <div className="w-2 h-2 rounded-full bg-yellow-500"></div>}
                </div>
                ...
                <div className="p-5">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="pt-3 border-t border-border/50 mt-3">
                            <p className="text-[10px] text-muted-foreground uppercase font-semibold mb-2">Occupants</p>
                            ...
                            {room.status === RoomStatus.OCCUPIED ? (
                                occupants.length > 0 ? (
                                    ...
                                    <div className="flex items-center gap-2 text-destructive text-xs">
                                        ...
                                        <span>Unknown Occupant</span>
                                    </div>
                                )
                            ) : (
                                <span className="text-xs text-muted-foreground italic">
                                    {room.status === RoomStatus.MAINTENANCE ? 'Under Maintenance' : 'Empty'}
                                </span>
                            )}
                        </div>
                    </div>
                ...
                <div className="bg-secondary/30 px-5 py-3 border-t border-border flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                        {isGhostOccupancy && (
                            <button 
                                onClick={() => handleFreeRoom(room.id!)}
                                title="Fix Status: Free Room"
