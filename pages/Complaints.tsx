...
const Complaints: React.FC = () => {
  const { data: complaints, loading, setData: setComplaints } = useComplaints();
  ...
  const updateStatus = async (id: string | number, status: 'Resolved' | 'In Progress') => {
    if (isDemo) {
      alert("Demo Mode: Cannot update complaint status.");
      return;
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        throw new Error("No rows deleted. Complaint may not exist or you don't have permission.");
      }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {complaints.length === 0 ? (
            ...
            complaints.map((c) => (
              <div 
                key={c.id} 
                className="bg-card p-5 rounded-xl border border-border shadow-sm flex flex-col justify-between hover:border-primary/30 transition-all"
              >
                <div>
